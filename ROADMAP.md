...
| 2 | Discovery | ✅ Done | Filesystem/language/project-type/framework/build-system/infrastructure/database/API-evidence/test-framework/secret detection, all read-only with confidence+evidence. `universal-test scan` implemented (text/json/markdown). Extended post-completion (2026-08-20) with C/C++ language detection, `c`/`cpp` project types, CMake/Meson/Bazel/Make/Conan/vcpkg build-system detection, and CTest/GoogleTest/Catch2/Unity test-framework detection — same evidence-first, manifest-anchored method as every other language. |
